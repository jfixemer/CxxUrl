...
static const uint8_t tbl[256] = { 0, 0, 0, 0, 0, 0, 0,
...
static const char* defaultPort(std::string& scheme)
{
    if (scheme == "http")
        return "80";
...
Url& Url::port(std::string p)
{
    ...
    // Don't explicity store the port for output if it is the
    // standard port of the scheme.
    if (p.empty() || p == defaultPort(m_scheme))
    {
        if (!m_port.empty())
        {
            m_port.clear();
...
{
    if (p.length() > 8000)
        throw Url::parse_error("Path exceeds limit of 8000 characters.");
...
void Url::parse_url(bool reparse) const
{
    if (!reparse && !m_parse)
        return;
    ...
    if (p == e)
    {
        if (!is_chars(b, p, 0x2F))
            throw Url::parse_error(
                "Path '" + std::string(b, p) + "' in '" + std::string(s, e - s) + "' is invalid");
        path_b = b;
        path_e = e;
    }
    else
...
void Url::build_url(std::ostream& url) const
{
    parse_url();
...
// Output
void Url::dump(std::ostream& o) const
{
    parse_url();
    if (!m_built)
        build_url();
